component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public MakeService function init() {
    	super.init( entityName="Make" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.Make
     */
    public CarTracker.model.orm.option.Make function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "MakeID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="MakeID" ) {
                c.isEq( "MakeID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
