component extends="CarTracker.model.service.BaseService" {
     ...
     * returns CarTracker.model.orm.option.Category
     */
    public CarTracker.model.orm.option.Category function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "CategoryID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="CategoryID" ) {
                c.isEq( "CategoryID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
