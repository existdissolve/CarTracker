component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public PositionService function init() {
    	super.init( entityName="Position" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.Position
     */
    public CarTracker.model.orm.option.Position function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "PositionID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="PositionID" ) {
                c.isEq( "PositionID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
