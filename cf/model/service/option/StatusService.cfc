component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public StatusService function init() {
    	super.init( entityName="Status" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.Status
     */
    public CarTracker.model.orm.option.Status function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "StatusID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="StatusID" ) {
                c.isEq( "StatusID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
