component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public UserRoleService function init() {
    	super.init( entityName="UserRole" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.UserRole
     */
    public CarTracker.model.orm.option.UserRole function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "UserRoleID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="UserRoleID" ) {
                c.isEq( "UserRoleID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
