component extends="CarTracker.model.service.BaseService" {
     ...
     */
    public CarTracker.model.orm.option.Color function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "ColorID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="ColorID" ) {
                c.isEq( "ColorID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
