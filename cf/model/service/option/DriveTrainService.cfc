component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public DriveTrainService function init() {
    	super.init( entityName="DriveTrain" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.DriveTrain
     */
    public CarTracker.model.orm.option.DriveTrain function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "DriveTrainID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="DriveTrainID" ) {
                c.isEq( "DriveTrainID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
