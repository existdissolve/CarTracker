component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public StaffService function init() {
    	super.init( entityName="Staff" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.Staff
     */
    public CarTracker.model.orm.Staff function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "StaffID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="StaffID" ) {
                c.isIn( "StaffID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
